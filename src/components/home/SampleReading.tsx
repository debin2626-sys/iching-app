    ...
    <section className="mt-24">
      ...
      >
        ...
        <div className="p-6 sm:p-8">
          ...
          {/* AI interpretation — collapsible: show first paragraph, expand for rest */}
          <div className="space-y-4">
            <p className="text-[#d4cfc5] text-sm leading-relaxed">
              {content.paragraphs[0]}
            </p>
            {expanded && content.paragraphs.slice(1).map((p, i) => (
              <p
                key={i}
          ...
          {content.paragraphs.length > 1 && (
            <button
              ...
              className="mt-4 flex items-center gap-1 mx-auto text-sm text-[#c9a96e] hover:text-[#e8d5a3] transition-colors"
            >
          ...
          <div className="text-center">
            <button
              onClick={() => router.push("/")}
              className="px-8 py-3 bg-[#c9a96e] text-[#0a0a12] text-base rounded-xl font-bold font-title tracking-wider transition-all duration-300 hover:bg-[#e8d5a3] hover:shadow-[0_0_20px_rgba(201,169,110,0.3)]"
            >
              {content.cta}
            </button>
          </div>
